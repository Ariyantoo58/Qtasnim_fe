...
const SimpleAreaChart = () => {
	...
	const [type, setTpe] = useState("MAX");
	const [picker, setPicker] = useState([
		"2020-04-30T17:00:00.000Z",
		"2030-06-07T17:00:00.000Z",
	]);
	...
	const handleType = async (e) => {
		const response = await axios.get(
			`http://localhost:8001/transactions/grouping?type=${e.value}&from=${picker[0]}&to=${picker[1]}`
		);
	...
	const handlePicker = async (e) => {
		const response = await axios.get(
			`http://localhost:8001/transactions/grouping?type=${type}&from=${e[0]}&to=${e[1]}`
		);
		...
		<Card>
			<CardHeader className="flex-sm-row flex-column justify-content-sm-between justify-content-center align-items-sm-center align-items-start">
				<div className="d-flex">
					<div tag="h4" className="my-auto">
						Data Terjual
					</div>
					<div>
						<Select
							className="form-control select bg-transparent border-0 shadow-none"
							options={[
								{
									value: "MAX",
									label: "Terbanyak",
